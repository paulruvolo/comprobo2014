...
class ParticleFilter:
    ...
    def __init__(self):
        ...
        t = time.time()
        self.occupancy_field = OccupancyField(map.map)
        print time.time() - t
    ...
    def update_particles_with_odom(self, msg):
        ...
        if self.current_odom_xy_theta:
            ...
            # atan gives a number from -pi/2 to pi/2-- need to incr to -pi to pi
                ...
                rotate_1 = alpha - old_odom_xy_theta[2]
                rotate_2 = delta[2] - rotate_1
                #print rotate_1,rotate_2

                hypotenuse = math.hypot(delta[0],delta[1])

                self.particle_cloud[i].theta += rotate_1
                self.particle_cloud[i].x     += hypotenuse*(math.cos(self.particle_cloud[i].theta))
                self.particle_cloud[i].y     += hypotenuse*(math.sin(self.particle_cloud[i].theta))
    ...
    def update_particles_with_laser(self, msg):
        ...
        print "I am here", zeroDegrees
        if zeroDegrees < 0.5 or zeroDegrees > 5.0:
            return
        # print zeroDegrees
        ...
        print self.particle_cloud[0].x+zeroDegrees*math.cos(self.particle_cloud[0].theta),self.particle_cloud[0].y+zeroDegrees*math.sin(self.particle_cloud[0].theta)
        zeroClosest = self.occupancy_field.get_closest_obstacle_distance (self.particle_cloud[0].x+zeroDegrees*math.cos(self.particle_cloud[0].theta),self.particle_cloud[0].y+zeroDegrees*math.sin(self.particle_cloud[0].theta))
        print zeroClosest
