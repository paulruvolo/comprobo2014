## This is Dara Behjat's code for the avoid obstacle behavior of the neato robot.
...
def avoid_obstacle(msg, pub):
    """sends angular and linear velocities to the robot depending on distance from obstacle"""
    global distance_to_wall
    velocity_msg = Twist(Vector3(0.2, 0.0, 0.0), Vector3(0.0, 0.0, 0.5))
    valid_measurements = []
    for i in range(5):
        if msg.ranges[i] > 0.0 and msg.ranges[i] < 1:
            valid_measurements.append(msg.ranges[i])
    if len(valid_measurements):
        velocity_msg = Twist(Vector3(0.0, 0.0, 0.0), Vector3(0.0, 0.0, 0.57))
    else:    
...
if __name__ == '__main__':
    pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10)
    sub = rospy.Subscriber('/scan', LaserScan, avoid_obstacle, pub)
    rospy.init_node('wall', anonymous=True)
    r = rospy.Rate(10) # 10hz
    try:
        wall()
    except rospy.ROSInterruptException: passollow
