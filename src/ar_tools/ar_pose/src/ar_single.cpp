...
// DEPRECATED: Fuerte support ends when Hydro is released
  ...
  void ARSinglePublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ...
    {
      cv::circle(capture_->image, cv::Point(marker_info[i].pos[0],marker_info[i].pos[1]), 5, cv::Scalar(0,0,255));
      for (int j = 0; j < 4; j++) {
          cv::circle(capture_->image, cv::Point(marker_info[i].vertex[j][0],marker_info[i].vertex[j][1]), 5, cv::Scalar(255,0,0));
      }
      std::cout << "marker_info pattern " << marker_info[i].id << " " << marker_info[i].pos[0] <<", " << marker_info[i].pos[1] << marker_info[i].cf << std::endl;
      if (marker_info[i].id == patt_id_) //} || marker_info[i].cf > 0.2)
      {
    ...
    if (k != -1)
    {
      ...
      // try to 
      pos[0] = arPos[2] * AR_TO_ROS;
      pos[1] = arPos[0] * AR_TO_ROS;
      pos[2] = arPos[1] * AR_TO_ROS;
      ...
      // this reordering is needed due to differences in AR_POSE coordinate conventions and how we are using the Tango
      // THIS WORKS SOMEWHAT FOR POSITIONS.... tf::Vector3 origin (pos[2], pos[0], pos[1]);
...
// DEPRECATED: Fuerte support ends when Hydro is released
      ...
      if(publishTf_)
      {
        ...
        {
          std::cout << "publishing reverse transform" << std::endl;
          ...
          broadcaster_.sendTransform(markerToCam);
            ...
            std::cout << "publishing normal transform" << std::endl;
